...
def build_answer_system_prompt() -> str:
    return (
        ...
        "factual ones.\n\n"
        "If the profile doesn't contain enough to genuinely answer the question (e.g. it asks for a "
        "specific province/state, a desired pay figure, or any other concrete fact simply not present in "
        "the profile), set answerable to false and answer to null — do NOT write a sentence explaining "
        "that you don't have enough information. That explanation is not a valid answer to put in a form "
        "field; a real applicant would leave the field blank and handle it themselves, not submit a "
        "paragraph about what their profile lacks. Setting answerable to false is what triggers exactly "
        "that safe fallback in the actual application.\n\n"
